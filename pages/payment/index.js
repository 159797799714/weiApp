...
Page({
  data: {
    ...
    isClicked: false,        // 是否已经点击，节流
    integralShow: false,      // 积分弹框显/隐
  },
  ...
  onUnload: function () {
    ...
    //   if (that.data.paymentData.product_list[0].point_exchange_num!=0 && that.data.paymentData.product_list[0].point_exchange_num){
    //     wx.redirectTo({
  ...
  paymentData: function(result) {
    ...
    if (result.err_code == 0) {
      ...
      console.log('paymentData', result.err_msg)
      this.setData({
  ...
  addressGo: function(e) { //点击头部我的地址-出现选择地址列表
    ...
    console.log('/pages/user/address/index?order_no=' + that.data.paymentData.order.order_no_txt + '&address=' + that.data.paymentData.wxapp_address.address_id + '&paymentPostage=' + that.data.paymentPostage)
