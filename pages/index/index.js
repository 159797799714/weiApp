...
Page({
  data: {
    ...
    code:'',
    userData: {},
    isShareToTogetherDialog: false,     //分享进入的  普通用户邀请新用户，走搜索授权流程
    mid: "",    //从微信充电小程序传来的代理商标识
    hideHongbaoGif: false,    //是否展示充电红包gif动画
    isCurEnter: false,
    shopHomeData: {},
    ...togetherDialogData,
  },
  onLoad: function(e) {
    var that = this;
    console.log('当前分享者是------', app.globalData.share_uid, '--------首页进入的参数------', e);
    let { share_uid, shareType, store_id, scene, room_id, custom_params, distance, cps_option, cps_type, mid } = e;
    let isCurEnter = scene ? true : (share_uid ? true : false);
    ...
    mid = mid || app.globalData.mid || '';

    if (distance) this.setData({ distance });
    ...
    if (room_id && custom_params){   //从直播间的商品跳转过来的  获取分享者的uid
      ...
      if (physical_id){
        ...
        common.post('app.php?c=lbs&a=switch_substore&type=1', { physical_id }, function(result) {
          ...
          setTimeout(function() {
            that.setData({ 'shopHomeData.store.physical_title': result.err_msg.name });
          }, 2000)
        }, '')
      }
    }
    if ((store_id == undefined || store_id == '') && app.globalData.store_id == '') {
    ...
    this.setData({ share_uid, store_id: app.globalData.store_id, mid, isCurEnter });

    publicFun.onLoad(app, this);  //授权弹框提前注册
    app.initLoginFun(this.checkUserType);    //进入页面登录判断用户是否为注册用户

    //好礼任务： 用户通过分享进入( 分享商品/邀请好友 )，改变分享者的积分任务状态
    if (share_uid&&cps_option&&cps_type) this.babyUserStatusChange(share_uid, cps_option, cps_type);

    //判断是否展示充电红包gif动画
    let _hideHongbaoGif = wx.getStorageSync('hideHongbaoGif');
    if (_hideHongbaoGif) this.setData({ hideHongbaoGif: true });
  ...
  onReady: function() {
    ...
    // common.post('app.php?c=store&a=index&store_id=' + app.globalData.store_id, '', "shopHomeData", that, '', true);
  ...
  onShow: function() {
    ...
    //获取数据
    let { custom_field_list } = this.data.shopHomeData, name = '';
    if (custom_field_list && custom_field_list.length && custom_field_list[0].field_type == "image_nav"){
      name = custom_field_list[0].content[this.data.currentTab].title;
    }
    common.post(`app.php?c=store&a=index&store_id=${app.globalData.store_id}&page_name=${name}`, '', "shopHomeData", that, '', true);


    this.getIncomeData();
    app.getTimeDifference();   //获取用户上次打开小程序距这次的时间差,超过30分钟，重新获取地理位置
    ...
    if (wx_ticket) {
      if (this.data.shopHomeData != '') publicFun.setUrl('');
    }else {
    ...
    //用户uid
    this.setData({ my_uid: app.globalData.my_uid });
    let _unlogin = wx.getStorageSync('unlogin') || app.globalData.unlogin;
    let shopName = wx.getStorageSync('shopName');
    if (_unlogin == undefined || !_unlogin || !shopName) return;
    that.setData({ _unlogin, shopName });
  },
  onHide: function() {
  ...
  onShareAppMessage: function(e) {
    var that = this;
    if (e.target && e.target.dataset.leader_id){
      let leader_id = e.target.dataset.leader_id;
      wx.setStorageSync('leader_id', leader_id);
    }
    this.setData({ haibaoCanvas: false, showSearch: false });

    let { nickname } = this.data.userData;
    let title = nickname ? `【${nickname}】` : '';
    console.log('首页分享-----nickname', nickname, '-----title----', title);
    return app.shareGetFans(`${title}邀请你一起0.01元秒好货，限时限量秒杀不等人，速速进入！`, ' ', `/pages/index/index`, 2, 'seckill');
  },
  onPullDownRefresh() {
    wx.showLoading({ title: '加载中', mask: true });
  ...
  successGetPhoneCallback(member_grade) {   //获取手机号码获取成后执行的操作
    ...
    let { share_uid } = app.globalData, { mid } = this.data;
    if (mid) return;

    if (member_grade < 1){
      this.setData({ isShareToTogetherDialog: false });
      if (!share_uid) this.getClipboardData('invitcode');    //搜索进入的, 注册及手机号授权完成后，检测粘贴板邀请码
      return;
    }

    let { isShareToTogetherDialog } = this.data;
    if (!share_uid || isShareToTogetherDialog){     //搜索进入的(老用户未授权手机号码)，或者 分享进来的，但是是普通用户邀请新用户isShareToTogetherDialog:true
      if (isShareToTogetherDialog) this.getClipboardData('invitcode');    //检测粘贴板
      this.setData({ isShareToTogetherDialog: false });

    }else {   //分享进来的， 授权成功后，弹合作弹框
      // this.setData({ 
      //   'commonModalData.showCommonModa': true,
      //   'commonModalData.type': 1
      // })
    }
  },
  checkUserType(res) {   //判断用户是否为新用户  is_new  1:表示新用户  null或者不存在: 表示注册用户
    ...
    if (!_inw){    //老用户 进入页面检测粘贴板内容，搜索商品
      //分享进入的 当前用户已经是注册用户以上级别 需要判断分析者和被分享者的会员级别
      if (share_uid&&share_uid != user.uid){
        this.getMemberBasicInfoByUserId('', user.uid);
      }else {
        // if (!_isnewStr) this.getClipboardData();    //老用户检测粘贴板    ---暂时不要了
      }
      return;
    }
  ...
  getMemberBasicInfoByUserId(isnew, curUserid, reget) {   //通过分享者的uid获取邀请人信息  不需要先登录也可以调用
    ...
    .then(res => {
      ...
      //邀请人是普通用户 无邀请码，1:他邀请普通用户不走授权流程  2:他邀请了新用户 走搜索授权流程-->弹合作弹框
      if (!reget && (!memberGrade || memberGrade < 1)){
        if(isnew) {
          me.setData({
            isShareToTogetherDialog: true,
            // 'commonModalData.showCommonModa': true,
            // 'commonModalData.type': 1
          })
        }
      ...
      console.log('首页是否为主入口------', this.data.isCurEnter, '--_jumpeAuth---', _jumpeAuth);
      if (_jumpeAuth && !this.data.isCurEnter) return;    //isCurEnter  --> 只有当前页面为主入口的时候才会保存这个变量
      wx.navigateTo({ url: `/pages/shareAuth/shareAuth?from=index` });
    })
    .catch(res => {
  ...
  getIncomeData(refresh) {   //获取用户信息
    ...
    .then(res => {
      ...
      //代理商逻辑： 已经登录的普通用户以下的用户，从充电那边进入的，自动注册
      let { member_grade } = res.data, { mid, _unlogin, _has_phone } = me.data;
      if (mid && member_grade < 1 && !_unlogin && _has_phone){
        console.log('获取用户信息----已经登录的普通用户---从充电过来----', mid, '----自动注册');
        me.toRegisterFun(() => {
          me.showMengChargeInitPoint(_has_phone);   //判断是否弹 猛犸充电用户的初始积分弹框
        })
        return;
      }
  ...
  showMengChargeInitPoint(_has_phone) {   //展示猛犸充电用户的初始积分弹框
    const me = this;
    let _userType = wx.getStorageSync('sg_user_type');   //当前用户类型： 0:普通用户  1:猛犸充电用户
    if (!_has_phone || !_userType) return;

    http_cps.get('history_points_inc', {})
    .then(res => {
      let { inc_points } = res.data;
      if (!inc_points){    //已经展示过初始化积分弹框， 就展示近期获得的充电积分
        me.chargePointsInc();   //获取猛犸充电用户新增积分
        return;
      }
      //展示猛犸充电用户初始积分弹窗 (8月1号-9月15日之间的充电积分)
      me.setData({
        'commonModalData.showCommonModa': true,
        'commonModalData.type': 7,
        'commonModalData.inc_points': inc_points
      })
    })
    .catch(res => {
      wx.showToast({ title: res.msg, icon: 'none' })
  ...
  chargePointsInc() {    //获取猛犸充电用户最近新增积分
    http_cps.get('charge_points_inc', {}, false)
    .then(res => {
      let { inc_points } = res.data;
      let _lately = wx.getStorageSync('lately_charge_point');
      if (!inc_points || _lately == inc_points) return;

      wx.setStorageSync('lately_charge_point', inc_points);   //保存近期积分，用作积分更新比较
      this.setData({
        'commonModalData.showCommonModa': true,
        'commonModalData.type': 6,
        'commonModalData.inc_points': inc_points
      })
    })
    .catch(res => {
  ...
  closeAni() {   //关闭充电红包gif动画
    this.setData({ hideHongbaoGif: true });
    wx.setStorageSync('hideHongbaoGif', true);
  },
  imageNavClickFun(e) {   //公共模板--image_nav点击事件
    let { index, name } = e.currentTarget.dataset;
    this.setData({ currentTab: index });

    common.post(`app.php?c=store&a=index&store_id=${app.globalData.store_id}&page_name=${name}`, '', "shopHomeData", this, '', false);
  },
  toRemindMe() {   //秒杀活动未开始，开启订阅通知 提醒我
    console.log('点击了提醒我')
    wx.requestSubscribeMessage({
      tmplIds: ['-gmPc_1zzx6eJ0nu5hsVBZF70WMjLTVw3ZhSuPy5-YU'],
      success: (res) => { 
        console.log(res);
      }
    })
  ...
  shopHomeData: function(result) {
    ...
    if (result.err_code == 0) {
      ...
      if (app.globalData.userInfo !== null && app.globalData.switch_store === false && this.data.physical_id === '') {
        ...
        common.post('app.php?c=lbs&a=switch_substore', data, shopHomeData, '', '', true);
      ...
      common.post('app.php?c=cart&a=number', '', "shoppingCatNum", that, '', true); //判断购物车数量
      // this.setData({
      ...
      for (var i in custom_field_list) {
        ...
        if (activities_modules.includes(custom_field_list[i].field_type) && custom_field_list[i].activities) {
          for (let activity of custom_field_list[i].activities) {
            ...
            if (start_time > currentDate && end_time > currentDate){    //活动未开始状态
              activity.buy_btn_name = '未开抢';
            }
            if (end_time < currentDate){    //活动已结束
              activity.buy_btn_name = '已结束';
            }
            activity.statusClass = statusClass
            
            //cps秒杀版本：计算当前售卖比例 total_quantity:总量  quantity:剩余件数
            activity.percentage = ((total_quantity - quantity)/total_quantity*100).toFixed(2) - 0;
          }
        }
      ...
      if (that.data.shopHomeData.store.physical_title == '') {
        ...
        common.post(url, location, 'getStoreCB', that, '', true)
      }else{
        //【ID1001217】用户登录后，更新门店信息
        var _url ='app.php?c=lbs&a=set_physical'
        common.post(_url, {}, 'upDataStore', that, '', true)
      }
      ...
      if (that.data.shopHomeData.show_head_type * 1 == 1) {
        ...
        common.post(groupbuyUrl, '', "showGroupbuy", that, '', true);
      }
      //首页分享弹窗开启；
      if (shopHomeData.open_dialog && shopHomeData.open_dialog == 1) { //弹
        ...
        common.post(url, sdata, "showWindows", that, '', true);
      } else { //不弹
        this.setData({
          lafenWindowsState: false,
        })
  ...
  getLeaderlistStatus() {   //获取团长信息及其是否有团长
    let that = this;
    ...
    common.post(url, params,"getLeaderDataFun", that, '', true)
  },
