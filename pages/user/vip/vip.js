...
Page({
  ...
  onShareAppMessage: function () {
    ...
    return getApp().shareGetFans('邀请你加入闪购Live商城', '', '/pages/index/index', 2, that.data.lafenImgurl);
  },
  showCanvasWind() { //生成海报
    //该店铺未设置拉粉商品
  ...
  getEwmCode() { //获取二维码 
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        //异步下载图片资源
        let ewmImgUrlDownLoad = '', canvasBgDownload = '';
        ewmImgUrlDownLoad = new Promise((resolve, reject) => { //下载二维码
          let url = res.data.err_msg
          wx.downloadFile({
            url: url,
            ...
            fail: res => {
              console.log("下载失败---ewmImgUrl---", url);
              wx.hideLoading();
              reject();
            }
          })
        ...
        //异步下载canvas的背景图片
        canvasBgDownload = new Promise((resolve, reject) => { //下载二维码
          let url = 'https://frontend-mam.oss-cn-hangzhou.aliyuncs.com/live/sharebg.jpg';
          wx.downloadFile({
            url,
            success: (res) => {
              if (res.statusCode === 200) {
                console.log(res);
                that.setData({
                  lafenBg: res.tempFilePath
                })
                resolve();
              }
            },
            fail: res => {
              console.log("下载失败----lafenBg---", url);
              wx.hideLoading();
              reject();
            }
          })
        ...
        let qCodeStatus = Promise.all([ewmImgUrlDownLoad, canvasBgDownload]);
        qCodeStatus.then(() => {
          that.makeCanvas();
          setTimeout(function () {
            let w = that.data.canvasWidth;
  ...
  makeCanvas() {   //生成一整张图片 + 二维码
    let that = this;
    let lafenBg = this.data.lafenBg;
    let canvasWidth = this.data.winWidth;
    let bili = canvasWidth / 635;
    let canvasHeight = 1100 * bili;
    ...
    //绘制二维码
    context.beginPath();
    context.arc(245 * bili, 1007 * bili, 68 * bili, 0, Math.PI * 2, false);
    context.setFillStyle("#fff");
    context.fill();
    context.clip(); //画好了圆 剪切  原始画布中剪切任意形状和尺寸。
    context.drawImage(this.data.ewmImgUrl, 180 * bili, 942 * bili, 130 * bili, 130 * bili);
    context.restore();
    ...
    context.draw({
      ...
      wx.canvasToTempFilePath({
        ...
        quality: 1.0,
        success: res => {
          this.setData({ lafenImgurl: res.tempFilePath });
          wx.hideLoading();
        },
        fail: fail => {
          wx.hideLoading();
