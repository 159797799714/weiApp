...
console.log('拿到第三方平台的数据extConfig------', extConfig);
...
common.post = function (url, param, callFun, that, errCallBack, isLoding) {
  ...
  wx.request({
    ...
    success: function(res) {
      ...
      //返回正常错误信息提示
      if ((res.data.err_code != 0 && res.data.err_code != 1 && res.data.err_code != 20000 && res.data.err_code != 10000) && res.data.err_code != 30001 && res.data.err_code != 1009 && res.data.err_code != 1010 && res.data.err_code != 404 && res.data.err_code != 1999) {
        // 过滤err_msg中html标签
        let pattern = /<\/?.*?>/g;
        let msg = res.data.err_msg ? res.data.err_msg.replace(pattern, ''):'';
        wx.hideLoading()
