...
var http_cps = require('./http_cps.js')
...
publicFun.oppenShopping = function(e, that) { //开启购物车遮罩层
  ...
  let product_id = current_tartget.product;
  let type = current_tartget.type;
  let buttonTxt = '下一步';
  if (type == 'add_cart') {
    buttonTxt = '加入购物车';
  }
  if (that.data.live_id && that.data.shoppingData.shoppingNum != undefined) {
    that.setData({
  ...
  if (that.data.live_id) {
    if (that.data.liveVideoData.status != 2) {
      data.live_id = that.data.live_id
    }
  }
  common.post('app.php?c=goods&a=info&product_id=' + product_id, data, shoppingData, '');
  ...
  function shoppingData(result) { //购物车数据
    ...
    if (result.err_code == 0) {
      ...
      if (result.err_msg.sku_list && result.err_msg.sku_list.length == 1) {
        ...
        vid = result.err_msg.property_list[0].values[0].vid
        //单规格如果有规格图片，设置规格图
      ...
      // 有多个规格是默认选中第一个规格
      else if(result.err_msg.sku_list && result.err_msg.sku_list.length > 1) { 
        vid = result.err_msg.property_list[0].values[0].vid,
        value = result.err_msg.property_list[0].values[0].value,
        pid = result.err_msg.property_list[0].pid;
      }
      
      console.log('xxxxxxxxxxvid', vid)
      that.setData({
        ...
        'shoppingData.shoppingCatData.product.value': value,
...
publicFun.shoppingVid = function(e, that) { //购物车商品规格选择
  ...
  let id = e.target.dataset.id,
    ...
    value = e.target.dataset.val ? e.target.dataset.val: '',
    image = e.target.dataset.image ? e.target.dataset.image: '';
...
publicFun.payment = function(that, e, page_name) { //去支付
  ...
  let memberGrade = e.target.dataset.grade;
  let is_add_cart = 0;
  let addType = 0;
  ...
  let use_point = e.target.dataset.way? e.target.dataset.way: 0
  common.post('app.php?c=order&a=add' + param + '&use_point=' + use_point, data, payment, '');
  ...
  function payment(result) { //去支付
    ...
    if (result.err_code == 0) {
      ...
      publicFun.paymentGo(order_no, page_name, memberGrade)
    }
  };


};
publicFun.paymentGo = function(order_no, page_name, memberGrade) { //去支付
  console.log('order_no', order_no, 'page_name', page_name)
  let gradeStr= memberGrade?'&memberGrade='+memberGrade: ''
  console.log('gradeStr', gradeStr)
  if (page_name) {
    wx.navigateTo({
      url: '/pages/payment/index?order_no=' + order_no + '&cfrom=' + page_name + gradeStr
    });
  } else {
    wx.navigateTo({
      url: '/pages/payment/index?order_no=' + order_no + gradeStr
    });
  }
...
publicFun.orderAgain = function(goodId) { // 再次购买
  console.log('publicFun再次购买，商品id', goodId)
  wx.navigateTo({
    url: '/pages/product/details?product_id=' + goodId
  })
};
...
publicFun.cancelOrder = function(that, order_no, index, callback) { //取消订单
  publicFun.promptMsg('确定取消此订单?', '确定', '取消', cancelOrder);
...
publicFun.addImg = function(that, evaluation, index) { //图片上传控件---评价
  ...
  //获取当前能上传的图片数量 最大为5张
  let max = typeof evaluation !== 'undefined' ? that.data.evaluationData.order_product_list[index].imgList.length : that.data.imgList.length;
  ...
  wx.chooseImage({ //图片上传控件
    ...
    success: function(res) { // 返回选定照片的本地文件路径列表，tempFilePath可以作为img标签的src属性显示图片
      ...
      var counts = 0;
      uploadFileFun(tempFilePaths[counts]);
          ...
          success: function(res) {
            ...
            if (res.err_code != 0 && res.err_code != 20000) {
              ...
              if (typeof evaluation !== 'undefined') {
                ...
                if (counts < tempFilePaths.length - 1){   //按顺序上传图片
                  counts++;
                  uploadFileFun(tempFilePaths[counts]);
                }
                return
...
publicFun.logistics = function(e, that) { //查询物流
  ...
  common.post('app.php?c=express&type=' + code + '&express_no=' + express, '', logistics, '', errCallBack);
  ...
  function errCallBack(result) {
    that.setData({ logisticsShow: false });
  }
};
...
publicFun.localExpress = function(that, integralChange) {
  ...
  // //是否自定义运费deliveryfeejoin=》》2、UU跑腿，1、达达，0、自定义
  // let deliveryfeejoin = that.data.paymentData.now_physical ? that.data.paymentData.now_physical.deliveryfeejoin : 0;
  // // deliveryfeejoin=1;
  // if (deliveryfeejoin > 0) {
  //   getThirdFreight(that, deliveryfeejoin);
  //   return;
  // } 
  // //获取当前订单的价格，如果不满足消费金额，则不能支付
  // let now_physical = that.data.paymentData.now_physical;
  ...
  // let show_mail = true;
  // if (currentDeliverShop.support_delivery && deliveryfeejoin == 0) {
  //   //重量配送邮费
  ...
  //     wx.showModal({
  //       title: '',
  //       content: '同城配送下收货详细地址需要通过定位获取，手动输入地址无效',
  //       showCancel: false,
  //       confirmText: '去设置',
  //       confirmColor: that.data.themeColorValue,
  //       success: function(res) {
  //         wx.navigateTo({
  //           url: '/pages/user/address/addressEdit?addid=' + address.address_id + '&order_no=' + that.data.paymentData.order.order_no_txt + '&address=' + address.address_id + '&paymentPostage=local'
  //         })
  //       }
  //     })
  ...
  // }
  // //获取第三方运费
  // function getThirdFreight(that, pttype){
  //   let data={};
  //   let paydata = that.data.paymentData;
  //   const { user_address,order } = paydata;
  //   data.store_id = getApp().globalData.store_id;
  //   data.pttype = pttype;//0无 1达达2uu跑腿
  //   data.order_no = order.order_no;//订单号
  //   data.rtel = user_address.tel;//收货人手机号
  //   data.rname = user_address.name;//收件人姓名
  //   data.address_id = user_address.address_id || 0;//收货地址id 如果没有值就传0
  //   data.address ={};
  //   data.address.province = user_address.province_txt;
  //   data.address.city = user_address.city_txt;
  //   data.address.area = user_address.area_txt;
  //   data.address.address = user_address.address;
  //   data.address.lat = user_address.lat;
  //   data.address.lng = user_address.lng;
  //   data.physical_id = order.physical_id;//门店id
  //   console.log("查询订单参数：", data);
  //   // that.setData({
  //   //   no_postage_text: "",
  //   //   postage: true,
  //   //   'paymentData.order.postage': 10
  //   // })
  //   // return false;
  //   common.post('app.php?c=order&a=dadaOrUuPostage', data, thirdFrightFee, '');
  //   function thirdFrightFee(result){
  //     if (result){
  //       if (result.err_code==0){
  //         // result.err_msg.postage>运费
  //         // result.err_msg.distance>距离 单位 米
  //         // result.err_msg.pttype>0无 1达达2uu跑腿
  //         //距离配送邮费
  //         that.setData({
  //           no_postage_text: "",
  //           postage: true,
  //           'paymentData.order.postage': result.err_msg.postage
  //         })
  //         //刷新运费
  //         publicFun.freight(that, that.data.paymentData.wxapp_address.address_id, 'local');
  //         return false;
      ...
      if (typeof(wx_data) != 'object') {
        ...
        setTimeout(function() {
          ...
          if (orderInfo.type == 6 && orderInfo.data_tuan_item_id) {
            ...
            console.log("zhifuxxxxxx", that.data)
            let cfrom = that.data.cfrom;
            if (cfrom) {
              // wx.redirectTo({
              //   url: '/pages/user/order/orderList?currentTab=all&currentTabShop=' + currentTabShop + '&cfrom=' + cfrom
              // });
              wx.redirectTo({
                // url: '/pages/POINT/pages/paySuccess/index?currentTab=all&currentTabShop=' + currentTabShop + '&cfrom=' + cfrom + '&type=paySuccess&memberGrade=' + memberGrade
                url: `/pages/POINT/pages/paySuccess/index?currentTab=all&currentTabShop=${currentTabShop}&cfrom=${cfrom}&type=paySuccess&memberGrade=${memberGrade}`
              })
            } else {
              // wx.redirectTo({
              ...
              // });
              wx.redirectTo({
                url: '/pages/POINT/pages/paySuccess/index?currentTab=all&currentTabShop=' + currentTabShop + '&type=paySuccess&memberGrade=' + memberGrade
              })
            }
          }
        }, 1000)
...
publicFun.getType = function(url, obj_nav) {
  ...
  if (obj_nav && (obj_nav.text == '直播' || obj_nav.text.indexOf('直播') != -1)) {
    r.type = 'switchTab';
    var reTag = /.*webapp\/wapindex\?id=(\d*).*/g;
    var id = url.replace(reTag, '$1');
    r.id = id;
    r.url = '/pages/livetab/livetab';
  }else if (obj_nav && (obj_nav.text == '会员' || obj_nav.text.indexOf('会员') != -1)) {
    r.type = 'switchTab';
    var reTag = /.*wap\/ucenter\.php\?id=(\d*).*/g;
    var id = url.replace(reTag, '$1');
    r.id = id;
    r.url = '/pages/membertab/membertab';
  }else if (url.includes('webapp/wapindex/#/intMall/')) {
    r.type = 'navigate'
    ...
    if (obj_nav && obj_nav.is_group1 == 1) {
      ...
      r.url = '/pages/index_xiaozhu/page?page_id=' + r.id;
    }
    ...
    var id = url.replace(reTag, '$1');
    r.id = id;
    r.url = '/pages/giftMember/giftuser/coupons?coupon_id=' + r.id;
...
publicFun.userCall = function(res, app, that) {
  ...
  wx.getUserInfo({
    ...
    success: function (res) {
      ...
      publicFun.getinfoCallFun(res, app, that, encryptedData, iv);
    },
    fail: function (_res){
      publicFun.getinfoCallFun(res, app, that);
    }
  })
}

/* 2020-09-02启用 去掉授权弹框 */
publicFun.getinfoCallFun = function (res, app, that, encryptedData,iv){
  let { wxapp_ticket, login, tym_open, loginData, invite_code, sms_code } = app.globalData;
  ...
  // 新增----------
  let login_info = wx.getStorageSync('str_login'), g_code = '';
  if (login_info) {
  ...
  data = {
    code: encodeURI(login.code || g_code),
    codes: encodeURI(login.code || g_code),
  ...
  if (tym_open && loginData.type == 2) {
    ...
    if (loginData.phone) {
      ...
      data.phone = app.globalData.new_numInput;
      data.invite_code = invite_code;

    }else {
      if (!(app.globalData.new_numInput && sms_code && invite_code)) {
        wx.hideLoading();
        wx.showModal({
          title: '提示',
  ...
  common.post('app.php?c=wxapp&a=store_login&from=publicUser', data, setUserInfo, '');

  function setUserInfo(result) {
    console.log("public登录返回参数;", result);
    let { err_code, err_msg } = result;
    if (err_code != 0){
      wx.hideLoading();
      if(that && that.data){
        that.setData({ showLoginModal:false });
    ...
    let { wxapp_ticket, user, unlogin, store_openid } = err_msg;
    app.globalData.my_uid = user.uid;

    //cps版本： 查看用户是否授权手机号码
    console.log('当前用户是否手机授过权------', user.phone);
    let _setph = user.phone ? 1 : 0;
    app.globalData.has_phone = _setph;
    wx.setStorageSync('has_phone', _setph);
    that.setData({ _has_phone: _setph });

    // wx.hideLoading();

    try {
      wx.setStorageSync('ticket', wxapp_ticket);
      app.globalData.wxapp_ticket = wxapp_ticket;
    } catch (e) { }
    ...
    if (unlogin != undefined) {
      app.globalData.unlogin = unlogin;
      wx.setStorageSync('unlogin', unlogin);
    }
    ...
    //cps版本--获取php返回的token
    http_cps.get('get_token', { ticket: wxapp_ticket, store_id: common.store_id, uid: user.uid })
    .then(res => {
      wx.setStorageSync('token_cps', res.data.token);
      wx.setStorageSync('sg_user_type', res.data.user_type);   //user_type: 0:普通用户  1:猛犸充电用户
      ofMerchantFun();
    })
    .catch(res => {
    ...
    //cps代理商逻辑: 充电用户进入的，自动注册为代理商的下级  is_open:是否打开挂载小柱名下 1:开 0:关
    function ofMerchantFun() {
      let _mid = that.data.mid || getApp().globalData.mid || '';
      if (!_mid || !user.is_open || !_setph){
        setRoutePage();
        return;
      }

      //从充电小程序过来的--->老用户，有手机号码，查询-->普通用户以下自动注册
      http.post('getMemberInfo', { userId: user.uid })
      .then(res => {
        let { memberGrade } = res.data;
        if (memberGrade < 1){
          console.log('充电用户-----', _mid, '----当前级别----', memberGrade, '----自动注册----');
          that.toRegisterFun(() => {
            setRoutePage();
          })
        }else {
    ...
    function setRoutePage() {    //处理页面是刷新还是跳转
      wx.hideLoading();
      var store_openid = user.store_openid || store_openid;
      if (store_openid) {
  ...
  function refresh_page(refreshConfig, config, liveId) {
    ...
    let _ph = that._has_phone || app.globalData.has_phone || wx.getStorageSync('has_phone');   //检测当前用户是否授权过手机号码
    console.log('昵称头像授权完成后----查看是否有手机号码', _ph);
    ...
    if (refreshConfig) {
      ...
      if (liveId){//直播列表页登录后直接跳转到详情页
        wx.navigateTo({
          url: '/pages/LIVEVIDEO/pages/liveVideo/liveVideoDetail?isShare=1&live_id=' + liveId,
        });
        return false;
      }
      if (refreshConfig.url) {//是否有地址
        wx.reLaunch({
          ...
          success() {
            if (_ph) return;
            getApp().wxloginMethods(() => {    //先重新登录一下，防止解密失败
              setTimeout(() => {
...
//       let { wxapp_ticket, user, unlogin, store_openid } = result.err_msg
//       app.globalData.my_uid = user.uid;
...
//       .then(res => {
//         let { token } = res.data;
//         wx.setStorageSync('token_cps', token);
//         ofMerchantFun();
//       })
//       .catch(res => {
//         console.log('获取token失败----', res);
...
//         //从充电小程序过来的--->老用户，有手机号码，查询-->普通用户以下自动注册
//         http.post('getMemberInfo', { userId: user.uid }, true)
//         .then(res => {
//           let { memberGrade } = res.data;
//           if (memberGrade < 1){
//             console.log('充电用户-----', _mid, '----当前级别----', memberGrade, '----自动注册----');
...
publicFun.onLoad = function(app, that) { //授权页面
  ...
  //点击昵称授权时，在后去到加密信息之前 先Loading处理
  that && (that.clickUserButtonFun = function(e){
    console.log('点击了昵称头像授权按钮--------', e);
    let { noregister } = e.currentTarget.dataset;   //比如点击我想换个邀请码: 点击了昵称授权头像，手机号码授权后，但不自动注册
    if (noregister)  wx.setStorageSync('change_invitcode', true);   //点击了换个邀请码  给个标识
    wx.showLoading({ title: '正在登录', mask: true });
  })
}

publicFun.setBarBgColor = function(app, that) { //修改导航条背景色
  console.log('执行了设置导航, that', that)
  ...
  if (bgcolor != '') {
    ...
    //自研cps版本增加： 不需要设置导航栏背景颜色
    console.log('getCurrentPages-----', getCurrentPages())

    let curpage = getCurrentPages() && getCurrentPages().length ? getCurrentPages().pop() : null;
    let pageFilter = [
      'pages/index/index', 
      'pages/gifttab/gifttab',
      'pages/couponProduct/couponProduct',
      'pages/my/my',
      'pages/postMoments/postMoments',
      "pages/productCpsDetail/productCpsDetail",
      "pages/shareAuth/shareAuth",
      "pages/getPhoneNumPage/getPhoneNumPage",
      "pages/seckill/index",
      "subPages/sub-fund/pages/cashout/cashout",
      "subPages/sub-fund/pages/cashRecord/cashRecord",
      "subPages/sub-order/pages/order-list/order-list",
      "subPages/sub-order/pages/order-detail/order-detail",
      "subPages/sub-order/pages/instruction/instruction",
      "subPages/sub-search/search/search",
      "subPages/sub-search/prolist/prolist",
      "subPages/sub-integral/pages/integral-detail/integral-detail",
      "subPages/sub-gifttab/pages/exchange-record/exchange-record",
      "subPages/sub-gifttab/pages/all-exchanges/all-exchanges",
      "subPages/sub-gifttab/pages/exc-goods-detail/exc-goods-detail",
      "subPages/sub-gifttab/pages/exc-goods-address/exc-goods-address"
    ]
    if (curpage && !pageFilter.includes(curpage.route)){
      tabBarStyle(bgcolor)
    }
...
publicFun.formatDuring = function(mss, cumFormat) {   //cumFormat以 小时:分钟:秒  展示
  var hours = parseInt((mss % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
  var minutes = parseInt((mss % (1000 * 60 * 60)) / (1000 * 60));
  var seconds = (mss % (1000 * 60)) / 1000;
  if (mss <= 24 * 60 * 60 * 1000) {
    return !cumFormat ? hours + "小时" + minutes + "分钟" + seconds + "秒" : (hours + ':' + minutes + ':' + seconds);
  } else {
    var days = parseInt(mss / (1000 * 60 * 60 * 24));
    return !cumFormat ? days + "天" + hours + "小时" + minutes + "分钟" + seconds + "秒" : (days + "天");
  }
}
...
//积分商品打开购物车
publicFun.oppenShoppingP = function(e, that) { //开启购物车遮罩层
  if (!getApp().isLoginFun(that)) { //判断用户是否登录
    return false;
  }
...
//将本地图片、网络图片、代码包路径转换为本地图片路径
publicFun.getImageInfoFun = function (path, callBack, isloading = false){
  ...
  wx.getImageInfo({
    ...
    fail(err) {
      isloading && wx.hideLoading();
      console.log("生成本地图片路径失败", err);
    }
  })
}

// 授权保存图片的方法
publicFun.checkAuthOfSaveImageToPhotosAlbum = function (callBack){
  wx.getSetting({
    success (res) {
      if (typeof res.authSetting['scope.writePhotosAlbum'] == 'undefined'){    //还没授权过，调起授权
        wx.authorize({
      ...
      if (res.authSetting['scope.writePhotosAlbum']){    //已经授权，并同意授权
        callBack && callBack(true);
        return;
      }

      settingAuth();   //授权过了，但没有同意授权

      function settingAuth() {
        wx.showModal({
          content: `申请授权保存图片或视频到你的相册`,
          confirmText: "去授权",
          cancelText: "取消",
          confirmColor: "#3296fa",
          success (res) {
            if (res.cancel) return;  //点击了取消按钮
            wx.openSetting({
              success(res) {
...
//给当前页面设置自定义tabar序号
publicFun.getTabBarFun = function(that, path) {
  let navlist = getApp().globalData.store_nav_list, _index = 0;
  navlist.forEach((item, index) => {
    if (path == item.pagePath || `/${path}` == item.pagePath){
      _index = index;
    }
  })
  if (typeof that.getTabBar === 'function' && that.getTabBar()) {
    that.getTabBar().setData({ selected: _index });
  }
}

// 节流
publicFun.stopClick = function (that, time = 2000) {
  console.log('进来了publicFun.stopClick', that.data.isClicked)
